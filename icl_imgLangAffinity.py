...
examples until the distribution is satisfied
"""
def load_split_batch(filenames, batch_size=None, balance=False):
    ...
    files = filenames
    if batch_size is not None and batch_size < len(filenames):
        files = r.sample(filenames, batch_size)
...
def train_hdn_img_to_hdn_joint(train_files, eval_files, learning_rate,
    ...
    with tf.name_scope('hidden_joint_2'):
        weights = weight_variable([NUM_TXT_FEATS, int(NUM_TXT_FEATS/2)])
        biases = bias_variable([int(NUM_TXT_FEATS/2)])
        hiddenJoint_2 = tf.nn.relu(tf.matmul(hiddenJoint_1, weights) + biases)
        retain_prob = tf.placeholder(tf.float32)
        hdn_joint_drop = tf.nn.dropout(hiddenJoint_2, retain_prob)
    with tf.name_scope('softmax'):
    ...
    if model_file is not None and (epochs is None or batch_size is None):
        ...
        for i in range(epochs):
            ...
            example_ids, batch_x_img, batch_x_txt, batch_y, = \
                load_split_batch(train_files, batch_size, balance)
            ...
            if dropout:
                dropout_prob = 0.5
            sess.run(train_step, feed_dict={x_img: batch_x_img, x_txt: batch_x_txt, y: batch_y, retain_prob: dropout_prob})
...
def init():
    ...
    #      arg_dict['batch_size'], arg_dict['balance_data'])
    tune(train_files, eval_files)

    #train_hdn_img_to_hdn_joint(train_files, eval_files,  arg_dict['learning_rate'],
