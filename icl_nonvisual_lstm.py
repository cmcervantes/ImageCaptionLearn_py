...
def setup(batch_size, lstm_hidden_width, start_hidden_width,
    ...
    with tf.variable_scope("softmax"):
        ...
        constant_epsilon = tf.constant([epsilon, epsilon], dtype=tf.float32)
    ...
    with tf.Session() as sess:
        ...
        for i in range(0, epochs):
            ...
            for j in range(0, n_iter):
                ...
                batch_tensors = nn_data.load_batch_first_last_sentence_mention(batch_mentions,
                                                                               data_dict, len(CLASSES))
...
def __init__():
    ...
    if arg_dict['train']:
        arg_dict['model_file'] = "/home/ccervan2/models/tacl201711/" + \
                                 nn_data.build_model_file_name(arg_dict, "nonvis_lstm")
    ...
    mention_idx_file = data_dir + "raw/" + data_root + "_mentions_nonvis.txt"
    feature_file = data_dir + "feats/" + data_root + "_nonvis.feats"
    feature_meta_file = data_dir + "feats/" + data_root + "_nonvis_meta.json"
    eval_sentence_file = data_dir + "raw/" + eval_data_root + "_captions.txt"
    eval_mention_idx_file = data_dir + "raw/" + eval_data_root + "_mentions_nonvis.txt"
    eval_feature_file = data_dir + "feats/" + eval_data_root + "_nonvis.feats"
